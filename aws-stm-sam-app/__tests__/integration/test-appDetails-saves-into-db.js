 ...
 */
describe("Test Function Invoke of appDetailsFunction", function () {
  let functionName;

  /**
   * Based on the provided stack name,
   * here we use cloudformation API to find out what the appDetailsFunction is
   */
  beforeAll(async () => {
    ...
    const functionResource = resources.find(
      (resource) => resource.LogicalResourceId === "appDetailsFunction"
    );
  ...
  /**
   * Calling the appDetails using AWS API
   */
  it("appDetails should be invokable", async () => {
